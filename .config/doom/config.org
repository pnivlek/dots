...
#+begin_src emacs-lisp
(setq doom-font (font-spec :family "Inconsolata_dz" :size 14)
...
#+begin_src emacs-lisp :tangle no
(defvar required-fonts '("Inconsolata_dz" "Overpass" "JuliaMono" "IBM Plex Mono" "Alegreya"))
...
Adding Tao theme as a good light theme
#+begin_src emacs-lisp
(package! tao-theme)
#+end_src
...
#+end_src
**** Activity Watcher
I track my window usage with ActivityWatch, and use the mode for emacs that
tells it what windows I'm spending my time on.
#+begin_src emacs-lisp
...
**** Tweaking defaults
#+begin_src emacs-lisp
(setq org-directory "~/.local/share/Dropbox/org"                      ; let's put files here
...
(setq org-agenda-files '("~/.local/share/Dropbox/org"))
...
**** Roam
***** Basic settings
Set in a separate subfolder within my dropbox directory. This and my org
directory are the only two things I track using dropbox.
...
#+begin_src emacs-lisp :noweb-ref none :tangle (if (file-exists-p "~/.local/share/Dropbox/roam/") "no" "packages.el")
