# Type: QssColor
c.colors.completion.category.bg = "{{ colors.background }}"

# Bottom border color of the completion widget category headers.
# Type: QssColor
c.colors.completion.category.border.bottom = "{{ colors.background }}"

# Top border color of the completion widget category headers.
# Type: QssColor
c.colors.completion.category.border.top = "{{ colors.background }}"

# Foreground color of completion widget category headers.
# Type: QtColor
c.colors.completion.category.fg = "{{ colors.foreground }}"

# Background color of the completion widget for even rows.
# Type: QssColor
c.colors.completion.even.bg = "{{ colors.background }}"

# Background color of the completion widget for odd rows.
# Type: QssColor
c.colors.completion.odd.bg = "{{ colors.background }}"

# Text color of the completion widget.
# Type: QtColor
c.colors.completion.fg = "{{ colors.foreground }}"

# Background color of the selected completion item.
# Type: QssColor
c.colors.completion.item.selected.bg = "{{ colors.green.normal }}"

# Bottom border color of the selected completion item.
# Type: QssColor
c.colors.completion.item.selected.border.bottom = "{{ colors.background }}"

# Top border color of the completion widget category headers.
# Type: QssColor
c.colors.completion.item.selected.border.top = "{{ colors.background }}"

# Foreground color of the selected completion item.
# Type: QtColor
c.colors.completion.item.selected.fg = "{{ colors.background }}"

# Foreground color of the matched text in the completion.
# Type: QssColor
c.colors.completion.match.fg = "{{ colors.yellow.normal }}"

# Color of the scrollbar in completion view
# Type: QssColor
c.colors.completion.scrollbar.bg = "{{ colors.background }}"

# Color of the scrollbar handle in completion view.
# Type: QssColor
c.colors.completion.scrollbar.fg = "{{ colors.green.normal }}"

# Background color for the download bar.
# Type: QssColor
c.colors.downloads.bar.bg = "{{ colors.background }}"

# Background color for downloads with errors.
# Type: QtColor
c.colors.downloads.error.bg = "{{ colors.red.normal }}"

# Foreground color for downloads with errors.
# Type: QtColor
c.colors.downloads.error.fg = "{{ colors.foreground }}"

# Color gradient stop for download backgrounds.
# Type: QtColor
c.colors.downloads.stop.bg = "{{ colors.cyan.normal }}"

# Color gradient interpolation system for download backgrounds.
# Type: ColorSystem
# Valid values:
#   - rgb: Interpolate in the RGB color system.
#   - hsv: Interpolate in the HSV color system.
#   - hsl: Interpolate in the HSL color system.
#   - none: Don't show a gradient.
c.colors.downloads.system.bg = 'none'

# Background color for hints. Note that you can use a `rgba(...)` value
# for transparency.
# Type: QssColor
c.colors.hints.bg = "{{ colors.yellow.normal }}"

# Font color for hints.
# Type: QssColor
c.colors.hints.fg = "{{ colors.background }}"

# Font color for the matched part of hints.
# Type: QssColor
c.colors.hints.match.fg = "{{ colors.blue.normal }}"

# Background color of the keyhint widget.
# Type: QssColor
c.colors.keyhint.bg = "{{ colors.background }}"

# Text color for the keyhint widget.
# Type: QssColor
c.colors.keyhint.fg = "{{ colors.foreground }}"

# Highlight color for keys to complete the current keychain.
# Type: QssColor
c.colors.keyhint.suffix.fg = "{{ colors.yellow.normal }}"

# Background color of an error message.
# Type: QssColor
c.colors.messages.error.bg = "{{ colors.red.normal }}"

# Border color of an error message.
# Type: QssColor
c.colors.messages.error.border = "{{ colors.red.normal }}"

# Foreground color of an error message.
# Type: QssColor
c.colors.messages.error.fg = "{{ colors.foreground }}"

# Background color of an info message.
# Type: QssColor
c.colors.messages.info.bg = "{{ colors.blue.normal }}"

# Border color of an info message.
# Type: QssColor
c.colors.messages.info.border = "{{ colors.blue.normal }}"

# Foreground color an info message.
# Type: QssColor
c.colors.messages.info.fg = "{{ colors.foreground }}"

# Background color of a warning message.
# Type: QssColor
c.colors.messages.warning.bg =  "{{ colors.foreground }}"

# Border color of a warning message.
# Type: QssColor
c.colors.messages.warning.border =  "{{ colors.foreground }}"

# Foreground color a warning message.
# Type: QssColor
c.colors.messages.warning.fg = "{{ colors.foreground }}"

# Background color for prompts.
# Type: QssColor
c.colors.prompts.bg = "{{ colors.background }}"

# # Border used around UI elements in prompts.
# # Type: String
c.colors.prompts.border = '1px solid ' + "{{ colors.background }}"

# Foreground color for prompts.
# Type: QssColor
c.colors.prompts.fg = "{{ colors.foreground }}"

# Background color for the selected item in filename prompts.
# Type: QssColor
c.colors.prompts.selected.bg = "{{ colors.magenta.normal }}"

# Background color of the statusbar in caret mode.
# Type: QssColor
c.colors.statusbar.caret.bg = "{{ colors.cyan.normal }}"

# Foreground color of the statusbar in caret mode.
# Type: QssColor
c.colors.statusbar.caret.fg = "{{ colors.background }}"

# Background color of the statusbar in caret mode with a selection.
# Type: QssColor
c.colors.statusbar.caret.selection.bg = "{{ colors.cyan.normal }}"

# Foreground color of the statusbar in caret mode with a selection.
# Type: QssColor
c.colors.statusbar.caret.selection.fg = "{{ colors.foreground }}"

# Background color of the statusbar in command mode.
# Type: QssColor
c.colors.statusbar.command.bg = "{{ colors.background }}"

# Foreground color of the statusbar in command mode.
# Type: QssColor
c.colors.statusbar.command.fg = "{{ colors.foreground }}"

# Background color of the statusbar in private browsing + command mode.
# Type: QssColor
c.colors.statusbar.command.private.bg = "{{ colors.background }}"

# Foreground color of the statusbar in private browsing + command mode.
# Type: QssColor
c.colors.statusbar.command.private.fg = "{{ colors.foreground }}"

# Background color of the statusbar in insert mode.
# Type: QssColor
c.colors.statusbar.insert.bg = "{{ colors.green.normal }}"

# Foreground color of the statusbar in insert mode.
# Type: QssColor
c.colors.statusbar.insert.fg = "{{ colors.background }}"

# Background color of the statusbar.
# Type: QssColor
c.colors.statusbar.normal.bg = "{{ colors.background }}"

# Foreground color of the statusbar.
# Type: QssColor
c.colors.statusbar.normal.fg = "{{ colors.foreground }}"

# Background color of the statusbar in passthrough mode.
# Type: QssColor
c.colors.statusbar.passthrough.bg = "{{ colors.blue.normal }}"

# Foreground color of the statusbar in passthrough mode.
# Type: QssColor
c.colors.statusbar.passthrough.fg = "{{ colors.foreground }}"

# Background color of the statusbar in private browsing mode.
# Type: QssColor
c.colors.statusbar.private.bg = "{{ colors.background }}"

# Foreground color of the statusbar in private browsing mode.
# Type: QssColor
c.colors.statusbar.private.fg = "{{ colors.foreground }}"

# Background color of the progress bar.
# Type: QssColor
c.colors.statusbar.progress.bg = "{{ colors.foreground }}"

# Foreground color of the URL in the statusbar on error.
# Type: QssColor
c.colors.statusbar.url.error.fg = "{{ colors.red.normal }}"

# Default foreground color of the URL in the statusbar.
# Type: QssColor
c.colors.statusbar.url.fg = "{{ colors.foreground }}"

# Foreground color of the URL in the statusbar for hovered links.
# Type: QssColor
c.colors.statusbar.url.hover.fg = "{{ colors.blue.normal }}"

# Foreground color of the URL in the statusbar on successful load
# (http).
# Type: QssColor
c.colors.statusbar.url.success.http.fg = "{{ colors.foreground }}"

# Foreground color of the URL in the statusbar on successful load
# (https).
# Type: QssColor
c.colors.statusbar.url.success.https.fg = "{{ colors.foreground }}"

# Foreground color of the URL in the statusbar when there's a warning.
# Type: QssColor
c.colors.statusbar.url.warn.fg = "{{ colors.red.normal }}"

# Background color of the tab bar.
# Type: QtColor
c.colors.tabs.bar.bg = "{{ colors.background }}"

# Background color of unselected even tabs.
# Type: QtColor
c.colors.tabs.even.bg = "{{ colors.background }}"

# Foreground color of unselected even tabs.
# Type: QtColor
c.colors.tabs.even.fg = "{{ colors.foreground }}"

# Color for the tab indicator on errors.
# Type: QtColor
c.colors.tabs.indicator.error = "{{ colors.red.normal }}"

# Color gradient start for the tab indicator.
# Type: QtColor
# c.colors.tabs.indicator.start = magenta

# Color gradient end for the tab indicator.
# Type: QtColor
# c.colors.tabs.indicator.stop = red

# Color gradient interpolation system for the tab indicator.
# Type: ColorSystem
# Valid values:
#   - rgb: Interpolate in the RGB color system.
#   - hsv: Interpolate in the HSV color system.
#   - hsl: Interpolate in the HSL color system.
#   - none: Don't show a gradient.
c.colors.tabs.indicator.system = 'none'

# Background color of unselected odd tabs.
# Type: QtColor
c.colors.tabs.odd.bg = "{{ colors.background }}"

# Foreground color of unselected odd tabs.
# Type: QtColor
c.colors.tabs.odd.fg = "{{ colors.foreground }}"

# Background color of selected even tabs.
# Type: QtColor
c.colors.tabs.selected.even.bg = "{{ colors.blue.normal }}"

# Foreground color of selected even tabs.
# Type: QtColor
c.colors.tabs.selected.even.fg = "{{ colors.background }}"

# Background color of selected odd tabs.
# Type: QtColor
c.colors.tabs.selected.odd.bg = "{{ colors.blue.normal }}"

# Foreground color of selected odd tabs.
# Type: QtColor
c.colors.tabs.selected.odd.fg = "{{ colors.background }}"

# Background color for webpages if unset (or empty to use the theme's
# color)
# Type: QtColor
c.colors.webpage.bg = "{{ colors.foreground }}"
